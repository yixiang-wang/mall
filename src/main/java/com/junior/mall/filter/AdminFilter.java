...
public class AdminFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        try {
            ...
            UserFilter.user.setId(jwt.getClaim(Constant.USER_ID).asInt());
            UserFilter.user.setUsername(jwt.getClaim(Constant.USER_NAME).asString());
            UserFilter.user.setRole(jwt.getClaim(Constant.USER_ROLE).asInt());
        ...
        if (!UserFilter.user.getRole().equals(2)) {
            PrintWriter out = new ServletResponseWrapper((HttpServletResponse) servletResponse).getWriter();
            out.write(ResponseUtils.error(MallExceptionEnum.NEED_ADMIN).toJsonString());
            out.flush();
            out.close();
            return;
        }
